...
public class OpenIntToExprHashMap implements Serializable {
	...
	private final IExpr missingEntries;
	 ...
	 */
	public OpenIntToExprHashMap(final IExpr missingEntries) {
	 ...
	 *          value to return when a missing entry is fetched
	 */
	public OpenIntToExprHashMap(final int expectedSize, final IExpr missingEntries) {
		// this.field = field;
	 ...
	 *          map to copy
	 */
	public OpenIntToExprHashMap(final OpenIntToExprHashMap source) {
		// field = source.field;
	...
	private void growTable() {
		...
		final IExpr[] newValues = buildArray(newLength);
	...
	private IExpr[] buildArray(final int length) {
		return new IExpr[length];
		// (T[]) Array.newInstance(field.getZero().getClass(), length);
	}
