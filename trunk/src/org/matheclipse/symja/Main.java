...
public class Main extends JApplet
{
  ...
  protected JPanel createCenter ()
  {
    ...
    center.add (createBottom (commandScroller), BorderLayout.SOUTH);

    DragSource.getDefaultDragSource ().
      createDefaultDragGestureRecognizer (commands, 1,
...
//    return center;
  ...
  protected JToolBar createToolbar ()
  {
    ...
    JButton insert =
      new JButton (new ImageIcon (getClass ().getResource ("Import24.gif")));
    JButton preferences =
      new JButton (new
		   ImageIcon (getClass ().getResource ("Preferences24.gif")));
    JButton about =
      new JButton (new ImageIcon (getClass ().getResource ("About24.gif")));
...
class ExecuteListener implements ActionListener
{
  ...
  public ExecuteListener (JTextField in,
			  ...
			  JScrollPane scroll, RootPaneContainer parent)
  {
    if (in == null)
...
class CommandExecutor extends Thread
{
  ...
  InfiniteProgressPanel progress;
    ...
    /** Creates a CommandExecutor to execute the command in the text
      ...
      * then hidden.
      */
  public CommandExecutor (DefaultListModel out,
			  JTextField source,
			  Scroller scroller, InfiniteProgressPanel progress)
  {
    in = source.getText ();
    this.out = out;
    this.source = source;
...
class ListDragListener implements DragGestureListener
{
  ...
  public void dragGestureRecognized (DragGestureEvent dge)
  {
    ...
    for (int counter = list.getFirstVisibleIndex ();
	 counter <= list.getLastVisibleIndex (); ++counter)
      {
	if (list.getCellBounds (counter, counter).
	    contains (dge.getDragOrigin ()))
	  {
	    text = new StringSelection ((String) list.getModel ().
