...
public class OutputFormFactory implements IConstantHeaders {
	...
	public void convertPlusOperator(final Writer buf, final IAST list, final InfixOperator oper, final int precedence)
			throws IOException {
		...
		for (int i = 1; i < list.size(); i++) {
			...
			if ((temp instanceof IAST) && (((IAST) temp).size() > 1) && ((IAST) temp).topHead().toString().equals(Times)) {
				...
				if ((temp1 instanceof ISignedNumber) && ((ISignedNumber) temp1).isNegative()) {
					if (((ISignedNumber) temp1).equalsInt(1)) {
						flag = true;
					} else {
						if (((ISignedNumber) temp1).equalsInt(-1)) {
							buf.write("-");
							...
							convertSignedNumber(buf, (ISignedNumber) temp1, oper.getPrecedence());
						}
					}
				} else {
					if (i > 1) {
	...
	public void convertSignedNumber(final Writer buf, final ISignedNumber o, final int precedence) throws IOException {
	...
	public void convert(final Writer buf, final Object o, final int precedence) throws IOException {
		...
		if (o instanceof ISignedNumber) {
			convertSignedNumber(buf, (ISignedNumber) o, precedence);
			return;
		}
