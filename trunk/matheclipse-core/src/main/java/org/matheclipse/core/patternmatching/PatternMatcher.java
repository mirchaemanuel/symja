...
public class PatternMatcher extends IPatternMatcher<IExpr> implements
  ...
  public class FlatMatcher {
    ...
    public boolean matchFlatList() {
      ...
      final KPartitionsIterator partitionIterator = new KPartitionsIterator(n,
          k);
      // save fPatternValuesArray in local variable
      final IExpr[] localPatternValuesArrayCopy = new IExpr[fPatternValuesArray.length];
      System.arraycopy(fPatternValuesArray, 0, localPatternValuesArrayCopy, 0,
          fPatternValuesArray.length);
      ...
      while ((fPartitionsIndex = partitionIterator.nextElement()) != null) {
        ...
        System.arraycopy(localPatternValuesArrayCopy, 0, fPatternValuesArray,
            0, fPatternValuesArray.length);
      }
    ...
    private boolean matchSingleFlatPartition() {
      ...
      for (int i = 1; i < fPartitionsIndex.length; i++) {
        ...
        if (partitionStartIndex + 1 == fPartitionsIndex[i]) {
          ...
          if (fOneIdentity) {
            if (!matchExpr(fLhsPatternList.get(partitionElementCounter + 1),
                lhsEvalList.get(partitionStartIndex + 1))) {
              return false;
            }
          } else {
  ...
  public class FlatOrderlessMatcher {
    ...
    public FlatOrderlessMatcher(final ISymbol sym, final IAST lhsPatternList,
        final IAST lhsEvalList) {
      fSymbol = sym;
      fLhsPatternList = lhsPatternList;
      this.lhsEvalList = lhsEvalList;
    ...
    public boolean matchFlatOrderlessList() {
      ...
      final int n = lhsEvalList.size() - 1;
      final int k = fLhsPatternList.size() - 1;
      final KPermutationsIterator permutationIterator = new KPermutationsIterator(
          lhsEvalList, n, 1);
      final KPartitionsIterator partitionIterator = new KPartitionsIterator(n,
          k);
    ...
    private boolean matchSingleFlatOrderlessPartition() {
      ...
      for (int i = 1; i < fPartitionsIndex.length; i++) {
        ...
        if (partitionStartIndex + 1 == fPartitionsIndex[i]) {
          ...
          if (fOneIdentity) {
            if (!matchExpr(fLhsPatternList.get(partitionElementCounter + 1),
                lhsEvalList.get(fPermutationsIndex[partitionStartIndex] + 1))) {
              return false;
            }
          } else {
            partitionElement = F.function(fSymbol);
            partitionElement.add(lhsEvalList
          ...
          for (int m = partitionStartIndex; m < fPartitionsIndex[i]; m++) {
            checkCanceled();
            if (m + 1 < fPartitionsIndex[i]) {
              if ((lhsEvalList.get(fPermutationsIndex[m + 1] + 1))
                  .isLTOrdered(lhsEvalList.get(fPermutationsIndex[m] + 1))) {
                // wrong ordering inside partitionElement
                return false;
              }
            }
  ...
  public class OrderlessMatcher {
    ...
    private boolean matchSingleOrderlessPermutation() {
      ...
      for (int i = 0; i < fPermutationsIndex.length; i++) {
        ...
        // OneIdentity check here
        // if (fOneIdentity) {
        if (!matchExpr(fLhsPatternList.get(i + 1), lhsEvalList
            .get(fPermutationsIndex[i] + 1))) {
          return false;
        }
        // } else {
  ...
  public boolean checkCondition() {
    if (fCondition != null) {
      ...
      try {
        final IExpr substConditon = EvaluationSupport.substituteLocalVariables(
            fCondition, fPatternSymbolsArray, fPatternValuesArray);
  ...
  public static boolean equivalent(final IExpr patternExpr1,
    ...
    if ((patternExpr1 instanceof IPattern)
        && (patternExpr2 instanceof IPattern)) {
   ...
   * @param pExpr
   */
  private int determinePatterns(final IExpr lhsPatternExpression,
      final HashMap<String, IExpr> patternMap) {
    if (lhsPatternExpression instanceof IAST) {
      ...
      if (lhsPatternExpression instanceof IPattern) {
        if (((IPattern) lhsPatternExpression).getSymbol() == null) {
          ...
          // for "named" patterns:
          final IPattern temp = (IPattern) patternMap.get(lhsPatternExpression
              .toString());
          if (temp != null) {
            ((IPattern) lhsPatternExpression).setIndex(temp.getIndex());
          } else {
            ((IPattern) lhsPatternExpression).setIndex(fPatternCounter++);
            fPatternSymbolsArray.add(((IPattern) lhsPatternExpression)
                .getSymbol());
            patternMap.put(lhsPatternExpression.toString(),
                lhsPatternExpression);
          }
        }
   ...
   * @return
   */
  protected boolean matchExpr(final IExpr lhsPatternExpression,
      final IExpr rhsExpression) {
    if (lhsPatternExpression instanceof IAST) {
  ...
  private boolean matchFlatList(final ISymbol sym, final IAST lhsPatternList,
      final IAST lhsEvalList) {
    if ((sym.getAttributes() & ISymbol.ORDERLESS) == ISymbol.ORDERLESS) {
      final FlatOrderlessMatcher foMatcher = new FlatOrderlessMatcher(sym,
          lhsPatternList, lhsEvalList);
      return foMatcher.matchFlatOrderlessList();
    } else {
      final FlatMatcher fMatcher = new FlatMatcher(sym, lhsPatternList,
          lhsEvalList);
      return fMatcher.matchFlatList();
    }
  }

  private boolean matchAST(final IAST lhsPatternExpression,
      final IExpr lhsEvalExpression) {
    if (lhsEvalExpression instanceof IAST) {
      ...
      if ((sym.getAttributes() & ISymbol.ORDERLESS) == ISymbol.ORDERLESS) {
        final OrderlessMatcher foMatcher = new OrderlessMatcher(
            lhsPatternExpression, lhsEvalList);
      ...
      for (int i = 1; i < lhsPatternExpression.size(); i++) {
        ...
        if ((temp instanceof IAST)
            && ((IAST) temp).isEvalFlagOn(IAST.CONTAINS_PATTERN)) {
          continue;
        }
