...
/**
 * Determine the coefficient of a univariate polynomial
 * 
 */
public class Coefficient extends AbstractFunctionEvaluator {
	...
	public IExpr evaluate(final IAST ast) {
		...
		return coefficient(ast);
	}

	protected static IExpr coefficient(final IAST ast) {

		ExprVariables eVar = new ExprVariables(ast.get(1));
		if (!eVar.isSize(1)) {
			// coefficient only possible for univariate polynomials
			return null;
		}
		...
		int exp = 1;
		if (ast.size() == 4) {
			exp = Validate.checkIntType(ast, 3, 0);
		}
		return rootsOfVariable(expr, variables, exp);
	}

	protected static IExpr rootsOfVariable(IExpr expr, IAST variables, long exp) {
		ASTRange r = new ASTRange(variables, 1);
		List<IExpr> varList = r.toList();
		try {
			JASConvert<BigRational> jas = new JASConvert<BigRational>(varList, BigRational.ZERO);
			GenPolynomial<BigRational> rPoly = jas.expr2Poly(expr);
			ExpVector ev = ExpVectorLong.create(1, 0, exp);
			rPoly.coefficient(ev);
	...
	private static IAST rootsOfPolynomial(GenPolynomial<BigRational> poly, JASConvert<BigRational> jas) {
		...
		for (SortedMap.Entry<GenPolynomial<BigRational>, Long> entry : map.entrySet()) {
			...
			if (varDegree <= 2) {
				...
				// // m = 2*a^3 - 9*a*b + 27* c
				// IInteger m =
				// C2.multiply(a.pow(3)).subtract(a.multiply(b.multiply(integer(9)))).add(c.multiply(integer(27)));
				// // k = a^2 - 3*b
