...
public class SparseFieldVector<T extends FieldElement<T>> implements FieldVector<T>, Serializable {
    ...
    public FieldVector<T> add(T[] v) throws IllegalArgumentException {
        ...
        for (int i = 0; i < v.length; i++) {
            res.setEntry(i, v[i].plus(getEntry(i)));
        }
        return res;
    }
    ...
    public T dotProduct(T[] v) throws IllegalArgumentException {
        ...
        while (iter.hasNext()) {
            ...
            res = res.plus(value.times(iter.value()));
        }
        return res;
     }
    ...
    public FieldVector<T> ebeDivide(FieldVector<T> v)
        ...
        while (iter.hasNext()) {
            iter.advance();
            res.setEntry(iter.key(), iter.value().div(v.getEntry(iter.key())));
        }
        return res;
    }
    ...
    public FieldVector<T> ebeDivide(T[] v) throws IllegalArgumentException {
        ...
        while (iter.hasNext()) {
            iter.advance();
            res.setEntry(iter.key(), iter.value().div(v[iter.key()]));
        }
        return res;
    }
    ...
    /** {@inheritDoc} */
     public FieldVector<T> ebeMultiply(T[] v) throws IllegalArgumentException {
        ...
        while (iter.hasNext()) {
            iter.advance();
            res.setEntry(iter.key(), iter.value().times(v[iter.key()]));
        }
        return res;
    }
     ...
     public FieldVector<T> mapDivideToSelf(T d) {
        ...
        while (iter.hasNext()) {
            iter.advance();
            entries.put(iter.key(), iter.value().div(d));
        }
        return this;
   }
     ...
     public FieldVector<T> mapInvToSelf() {
        for (int i = 0; i < virtualSize; i++) {
            setEntry(i, field.getOne().div(getEntry(i)));
        }
        return this;
   }
     ...
     public FieldVector<T> mapSubtractToSelf(T d) {
        return mapAddToSelf(field.getZero().minus(d));
    }
    ...
    public FieldMatrix<T> outerProduct(T[] v) throws IllegalArgumentException {
        ...
        while (iter.hasNext()) {
            ...
            for (int col = 0; col < virtualSize; col++) {
                res.setEntry(row, col, value.times(v[col]));
            }
        }
        return res;
     }
    ...
    throws IllegalArgumentException {
        checkVectorDimensions(v.getDimension());
        return v.mapMultiply(dotProduct(v).div(v.dotProduct(v)));
    }
    ...
    public SparseFieldVector<T> subtract(SparseFieldVector<T> v) throws IllegalArgumentException{
        ...
        while (iter.hasNext()) {
            ...
            int key = iter.key();
            if (entries.containsKey(key)) {
                res.setEntry(key, entries.get(key).minus(iter.value()));
            } else {
                res.setEntry(key, field.getZero().minus(iter.value()));
            }
        }
        return res;
    }
    ...
    public FieldVector<T> subtract(T[] v) throws IllegalArgumentException {
        ...
        for (int i = 0; i < v.length; i++) {
            if (entries.containsKey(i)) {
                res.setEntry(i, entries.get(i).minus(v[i]));
            } else {
                res.setEntry(i, field.getZero().minus(v[i]));
            }
        }
        return res;
    }
