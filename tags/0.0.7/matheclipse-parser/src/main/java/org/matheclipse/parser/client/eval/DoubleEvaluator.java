...
public class DoubleEvaluator {
	...
	static class SetFunction implements IDoubleFunction {
		public double evaluate(DoubleEvaluator engine, FunctionNode function) {
			...
			double result = engine.evaluateNode((ASTNode) function.getNode(2));
	...
	static class MaxFunction implements IDoubleFunction, IDouble2Function {
		...
		public double evaluate(DoubleEvaluator engine, FunctionNode function) {
			...
			int end = function.size();
			for (int i = 1; i < end; i++) {
				temp = Math.max(result, engine.evaluateNode((ASTNode) function.getNode(i)));
				if (temp > result) {
					result = temp;
				}
			}
	...
	static class MinFunction implements IDoubleFunction, IDouble2Function {
		...
		public double evaluate(DoubleEvaluator engine, FunctionNode function) {
			...
			int end = function.size();
			for (int i = 1; i < end; i++) {
				temp = Math.min(result, engine.evaluateNode((ASTNode) function.getNode(i)));
				if (temp < result) {
					result = temp;
				}
			}
	...
	public double evaluateNode(final ASTNode node) {
		...
		if (node instanceof SymbolNode) {
			IDoubleValue v = (IDoubleValue) fVariableMap.get(node.toString());
			if (v != null) {
				return v.getValue();
			}
			Double dbl = (Double) SYMBOL_DOUBLE_MAP.get(node.toString());
			if (dbl != null) {
			...
			if (symbol.equals("If")) {
				...
				if (functionNode.size() == 1) {
					...
					if (obj instanceof IDouble1Function) {
						return ((IDouble1Function) obj).evaluate(evaluateNode((ASTNode) functionNode.getNode(1)));
					}
				} else if (functionNode.size() == 3) {
					if (obj instanceof IDouble2Function) {
						return ((IDouble2Function) obj).evaluate(evaluateNode((ASTNode) functionNode.getNode(1)),
		...
		if (functionNode.size() > 0 && functionNode.getNode(0) instanceof SymbolNode) {
			...
			if (functionNode.size() == 2) {
				...
				if (obj instanceof IBooleanDouble2Function) {
					return ((IBooleanDouble2Function) obj).evaluate(evaluateNode((ASTNode) functionNode.getNode(1)),
							evaluateNode((ASTNode) functionNode.getNode(2)));
