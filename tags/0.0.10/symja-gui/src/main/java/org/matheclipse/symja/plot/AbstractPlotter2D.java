...
/** The parent of all other two-dimensional Plotters.
  *
  * Note that, although the code refers to the horizontal axis as the
  * "X" axis, and the vertical axis as the "Y" axis, the names and
  * associated labels displayed can be changed by the user at
  * runtime, regardless of the variable names used by the code.
  */

public abstract class AbstractPlotter2D extends JComponent
                                        implements Runnable {
    ...
    /** The default colors used, unless explicitly overridden.
      */
    protected static Color COLOR[] = new Color[]{Color.BLUE,
                                       Color.RED,
                                       Color.GREEN,
                                       Color.ORANGE,
                                       Color.MAGENTA,
    ...
    /** Sets the resolution (number of points plotted) on future
      * plots. Does not affect existing plots that are already
      * displayed.
      */
    ...
    /** Creates a new plotter and initializes the frame associated
      * with it.
      */
    ...
    /** Paints the functions on the screen..
      */
    protected abstract void paintPlots(Graphics2D g2d,
                                       int top,
                                       int height,
                                       int bottom,
                                       int left,
    ...
    protected void setupText() {
        ...
        ascent = (int)new TextLayout(Double.toString(yMax),
                                     f,
                                     frc).getAscent();
    ...
    /** Fills the background and draws axes.
      */
    protected void fillBackground(Graphics2D g2d,
                                  int xAxis,
                                  int yAxis,
                                  int top,
                                  int bottom,
    ...
    protected void paintAxisLabels(Graphics2D g2d,
                                   ...
                                   int left,
                                   int right) {
        Shape yArrow = new Polygon(new int[] {yAxis,
                                              yAxis - ascent / 2,
                                              yAxis,
                                              yAxis + ascent / 2},
                                   new int[] {ascent,
        ...
        g2d.drawGlyphVector(yLabel,
                            yAxis - (yLabelLength / 2),
                            ascent);

        g2d.setPaint(new GradientPaint(right,
                                       0,
                                       Color.WHITE,
                                       right + ascent,
                                       0,
                                       Color.BLACK));
        g2d.fill(new Polygon(new int[] {right,
                                        right + ascent,
                                        right,
                                        right + ascent / 3},
                             new int[] {xAxis - ascent / 2,
                                        xAxis,
                                        xAxis + ascent / 2,
                                        xAxis},
                             4));
        ...
        g2d.drawGlyphVector(yMaxText,
                            yAxis - ascent - yMaxLength,
                            top + ascent / 2);
        g2d.drawGlyphVector(yMinText,
                            yAxis - ascent - yMinLength,
                            bottom + ascent / 2);
        ...
        g2d.drawGlyphVector(xMaxText,
                            right - xMaxLength / 2,
                            xAxis + ascent * 2);
        g2d.drawGlyphVector(xMinText,
                            left - xMinLength / 2,
                            xAxis + ascent * 2);
    }
