...
public abstract class AbstractPlotter2D extends JPanel { // implements
	 ...
	 */
	final static Color COLOR[] = new Color[] { Color.BLUE, Color.RED, Color.GREEN, Color.ORANGE, Color.MAGENTA, Color.PINK,
	 ...
	 */
	public abstract void plot(IAST args);
	...
	public void paint(final Graphics g) {
		...
		final int width = getDrawableWidth(left, right);
		final int height = getDrawableHeight(top, bottom);
		final int xAxis = xAxis(top, height);
		final int yAxis = yAxis(left, width);
	 ...
	 */
	protected void fillBackground(final Graphics2D g2d, final int xAxis, final int yAxis, final int top, final int bottom,
	 ...
	 */
	protected void paintAxisLabels(final Graphics2D g2d, final int xAxis, final int yAxis, final int top, final int bottom,
			final int left, final int right) {
		final Shape yArrow = new Polygon(new int[] { yAxis, yAxis - ascent / 2, yAxis, yAxis + ascent / 2 }, new int[] { ascent,
				ascent * 2, ascent * 5 / 3, ascent * 2 }, 4);
	 ...
	 */
	protected int getDrawableHeight(final int top, final int bottom) {
		return bottom - top;
	}
	...
	protected int getRight() {
		final int right = getWidth() - (ascent + xLabelLength);
	 ...
	 */
	protected int getDrawableWidth(final int left, final int right) {
		return right - left;
	}
	 ...
	 */
	protected int yAxis(final int left, final int width) {
	 ...
	 */
	protected int xAxis(final int top, final int height) {
