...
//	};
	 ...
	 * Holds the dividend.
	 */
	private BigInteger _dividend;

	/**
	 * Holds the divisor.
	 */
	private BigInteger _divisor;
...
//		} else {
			r =  new Rational();
//		}
		r._dividend = BigInteger.valueOf(dividend);
		r._divisor = BigInteger.valueOf(divisor);
		return r.normalize();
	}
...
//		} else {
			r =  new Rational();
//		}
		r._dividend = rat._dividend;
		r._divisor = rat._divisor;
		return r;
	}
	...
	/**
	 * Returns the smallest divisor of the fraction representing this rational
	 * (always positive).
	 * 
	...
	public Rational plus(Rational that) {
		return Rational.valueOf(this._dividend.times(that._divisor).plus(this._divisor.times(that._dividend)),
	...
	public Rational minus(Rational that) {
		return Rational.valueOf(this._dividend.times(that._divisor).minus(this._divisor.times(that._dividend)),
				this._divisor.times(that._divisor)).normalize();
	}
	...
	public Rational inverse() {
		...
		return _dividend.isNegative() ? Rational.valueOf(_divisor.opposite(), _dividend.opposite()) : Rational.valueOf(_divisor,
				_dividend);
	}
	 ...
	 */
//	public Text toText() {
//		return _dividend.toText().concat(Text.valueOf('/')).concat(_divisor.toText());
//	}
	...
	public double doubleValue() {
		...
		if (dividendBitLength > divisorBitLength) {
			...
			BigInteger dividend = _dividend.shiftRight(shift);
			...
			BigInteger divisor = _divisor.shiftRight(shift);
	...
	private Rational normalize() {
		if (!_divisor.isZero()) {
			if (_divisor.isPositive()) {
				BigInteger gcd = _dividend.gcd(_divisor);
...
//	}
	...
	public Rational copyNew() {
		...
		r._dividend = _dividend.copyNew();
		r._divisor = _divisor.copyNew();
		return r;
	}
