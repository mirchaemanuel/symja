...
public class JASConvert<C extends RingElem<C>> {
	private final RingFactory fRingFactory;
	...
	public JASConvert(final List<? extends IExpr> variablesList, RingFactory ringFactory, TermOrder termOrder) {
		...
		this.fPolyFactory = new GenPolynomialRing(fRingFactory, fVariables.size(), fTermOrder, vars);
		this.fBigIntegerPolyFactory = new GenPolynomialRing(new edu.jas.arith.BigInteger(0), fVariables.size(), fTermOrder, vars);
	}
	...
	public GenPolynomial<C> expr2Poly(final IExpr exprPoly) throws ArithmeticException, ClassCastException {
		if (exprPoly instanceof IAST) {
			...
			BigInteger bi = ((IInteger) exprPoly).getBigNumerator().toJavaBigInteger();
