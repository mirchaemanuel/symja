...
public class Symbol extends ExprImpl implements ISymbol {
  ...
  public void pushLocalVariable(final IExpr expression) {
    final Stack<IExpr> localVariableStack = EvalEngine
        .localStackCreate(fSymbolName);
  ...
  public boolean equals(final Object obj) {
    ...
    if (fSymbolName.equals(((Symbol) obj).fSymbolName)) {
      if (Config.DEBUG) {
        System.err.println(fSymbolName + " EQUALS "
            + ((Symbol) obj).fSymbolName);
      }
      return true;
    }
    return false;
  ...
  public IExpr evalDownRule(final IEvaluationEngine ee, final IExpr expression) {
    ...
    if ((fSimplePatternRules != null) && (expression instanceof IAST)) {
      final Integer hash = Integer.valueOf(((IAST) expression)
          .patternHashCode());
  ...
  public PatternMatcher putDownRule(ISymbol setSymbol, final boolean equalRule,
      final IExpr leftHandSide, final IExpr rightHandSide,
      final IExpr condition, final int priority) {
    if (Config.DEBUG) {
    ...
    final PatternMatcherAndEvaluator pmEvaluator = new PatternMatcherAndEvaluator(
        setSymbol, leftHandSide, rightHandSide);
  ...
  private PatternMatcher addSimplePatternRule(final IExpr leftHandSide,
      ...
      if (ast.size() > 1) {
        if (header == F.Power && ast.size() == 3) {
          if (ast.get(1) instanceof ISymbol) {
            final int cp = fSymbolName
                .compareTo(((Symbol) ast.get(1)).fSymbolName);
            if (cp != 0) {
              return cp;
            }
